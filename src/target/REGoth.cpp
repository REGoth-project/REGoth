...
//		bgfx::init(args.m_type, args.m_pciId);
        ...
        console.registerCommand("goto waypoint", [this](const std::vector<std::string>& args) -> std::string {
            ...
            std::string waypointArgument = args[2];
            std::transform(waypointArgument.begin(), waypointArgument.end(), waypointArgument.begin(), ::toupper);
            ...
            const World::Waynet::Waypoint point = waynet.waypoints.at(waypointIndex->second);
            ...
            Vob::setPosition(player, point.position);
