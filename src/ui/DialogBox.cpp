...
void DialogBox::onInputAction(EInputAction action)
{
    ...
    switch(action)
    {
        case IA_Up: m_CurrentlySelected = std::max(0, std::min(((int)m_Choices.size()) - 1, m_CurrentlySelected - 1)); break;
        case IA_Down: m_CurrentlySelected = std::max(0, std::min(((int)m_Choices.size()) - 1, m_CurrentlySelected + 1));break;
        case IA_Left:break;
        case IA_Right:break;
